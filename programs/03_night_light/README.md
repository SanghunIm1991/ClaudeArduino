...
**TO-220パッケージのピン配置（刻印面を正面にして脚を下に向けたとき、左からG・D・S）**は多くのIRFシリーズTO-220品で共通の一般的な配置だが、正確な配置は現物のデータシート表記に従うことを推奨する。特にG（ゲート）とD/S（ドレイン/ソース）を取り違えるとMOS-FETが正しく導通/遮断せずLEDが常時薄く点灯する等の誤動作につながるため、配線後の動作確認（後述の「ビルド・書き込み・動作確認手順」）で必ず挙動を確かめること。
